...
open class WeaponRepository {
    ...
    fun findAll(): CompletableFuture<List<Weapon>> {
        val completableFuture = CompletableFuture<List<Weapon>>()
        CoroutineScope(Dispatchers.IO).launch {
            transaction {
                ...
                completableFuture.complete(weapons)
            }
        }
        return completableFuture
    }
    ...
    fun findById(id: Long): CompletableFuture<Weapon> {
        val completableFuture = CompletableFuture<Weapon>()
        CoroutineScope(Dispatchers.IO).launch {
            transaction {
                ...
                completableFuture.complete(weapon)
            }
        }
        return completableFuture
    }
